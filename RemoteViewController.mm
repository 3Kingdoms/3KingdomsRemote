...
- (id)initWithNetService:(NSNetService*) netService
{
    ...
    _addrCount = [addresses count];
    if (_addrCount > REMOTE_VIEW_CONTROLLER_MAX_ADDRESSES) _addrCount = REMOTE_VIEW_CONTROLLER_MAX_ADDRESSES;
...
- (void) sendIdRequest
{
    ...
    if (_newStyle and uidString != nil) dName = [NSString stringWithFormat:@"%@#%@", [UIDevice currentDevice].name, uidString];
    ...
    //NSUUID *idVal = [UIDevice currentDevice].identifierForVendor;
    //cout << "HELLO WORLD" << [dName UTF8String] << endl;
    
//    int dLen = [dName length];
//    if (dLen > 100) dLen = 100;
//    char *b = buffer;
//    // just take raw ascii values.. (sorry, rest of the world)
//    for (int i = 0; i < dLen; i++){
//        int val = [dName characterAtIndex:i];
//        if (val < 128){
//            *b = val;
//            b++;
//        }
//        // convert funky apostrophe so we at least can get "Jim Bob's iPhone" on the other end
//        // FIXME remove this once we can support full UTF8..
//        else if (val == 8217){
//            *b = '\'';
//            b++;
//        }
    ...
    for (unsigned int i = 0; i < _addrCount; i++){
        ...
        *(short*)&data[2] = _idRequestKey;
        ...
        if (sa->sa_family == AF_INET6) s = _socket6;
            ...
            // jasonhu5
            // NSLog(@"Using ipv4");
            // NSLog(@"Addr: %s", (sockaddr*)&_addresses[i]);
            //
            s = _socket4;
        }
        else abort();
        
        int err = sendto(s,data,5+nameLen,0,(sockaddr*)&_addresses[i],_addressSizes[i]);
...
- (void) process
{
    ...
    // if we havn't heard from the server in a while, time out
    // FIXME: i think sometimes when becoming active this can get called without
    // lastContactTime getting reset properly first..
//    if (t - _lastContactTime > 10.0){
//        //printf("server timed out\n");
//        [AppController debugPrint:@"Connection Timed Out."];
//        [self tryToDie];
//    }
}
...
{
    {
        ...
        int amt = recvfrom(s,buffer,sizeof(buffer),0,(sockaddr*)&addr,&l);
            ...
            switch(buffer[0]){
                case BS_REMOTE_MSG_ID_RESPONSE:
                {
                    if (amt == 3){
                        ...
                        if (!_waitingForIDResponse) break;
                        ...
                        {              
                            ...
                            ip4 = (struct sockaddr_in *) ((sockaddr*)&addr);
//                            NSLog(@"%s:%d", inet_ntoa(ip4->sin_addr), ip4->sin_port);
                            _myServerAddr = ip4->sin_addr;
                            //
                            _connected = YES;
                        ...
                        // we said we support protocol v2.. if they respond with 100, they do too.
                ...
                case BS_REMOTE_MSG_STATE_ACK:
                {
                    if (amt == 2){
                        ...
                        if (stateDiff > 0 and stateDiff < 128){
                            ...
                            if (lag > _currentLag) _currentLag = lag;
                            _requestedState++;
...
- (void) shipUnAckedStatesV2
{
    ...
    if (_id != -1){
        ...
        if (_haveV4){
            int err = send(_socket4,data,4+3*statesToSend,0);
            if (err == -1) NSLog(@"ERROR %d on v4 sendto\n",errno);
        }
        if (_haveV6){
            int err = send(_socket6,data,4+3*statesToSend,0);
...
- (void) shipUnAckedStatesV1
{
    ...
    //cout << "SHIPPING UNACKED STATES" << endl;
    ...
    if (_id != -1){
        ...
        //cout << "SENDING " << int(statesToSend) << " STATES" << endl;

        //printf("sending %d states; %d thru %d at time %f\n",statesToSend,s,_nextState-1,CACurrentMediaTime());
        ...
        if (_haveV4){
            int err = send(_socket4,data,4+2*statesToSend,0);
            if (err == -1) NSLog(@"ERROR %d on v4 sendto\n",errno);
        }
        if (_haveV6){
            int err = send(_socket6,data,4+2*statesToSend,0);
            if (err == -1) NSLog(@"ERROR %d on v6 sendto\n",errno);
        }
    }    
...
- (void)viewDidLoad {
    ...
    _lagMeter.textAlignment = UITextAlignmentCenter;
    ...
    // buttons
    {
        ...
        float offs = fBase.size.width*0.5*offset;
        float ins = (fBase.size.width-buttonSize)*0.5;
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        ...
        b.frame = CGRectMake(buttonWidth*0.6*3.9,5,buttonWidth*0.6,buttonHeight*0.6);
        b.autoresizingMask = UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleRightMargin;
    }
    
    [b addTarget:self action:@selector(showMission) forControlEvents:UIControlEventTouchUpInside];
    [b setBackgroundImage:[UIImage imageNamed:@"mission.png"] forState:UIControlStateNormal];
    [self.view addSubview:b];
...
static void receiveData(CFSocketRef cfSocket, CFSocketCallBackType type, CFDataRef address, const void *data, void *info)
{
    RemoteViewController *rvc = (RemoteViewController*)info;
    int s = CFSocketGetNative(cfSocket);
    if (s){
//        [rvc readFromSocket:s];
        NSLog(@"Got something from BS!");
//        unsigned char buffer[10];
//        sockaddr_in6 addr;
//        socklen_t l = sizeof(addr);
//        int amt = recvfrom(s,buffer,sizeof(buffer),0,(sockaddr*)&addr,&l);
...
//static void receiveData(CFSocketRef s,
...
- (void) showMission
{
    // make connection to server and retrieve mission
    
    CFSocketRef s = CFSocketCreate(NULL, PF_INET,
                                   SOCK_STREAM, IPPROTO_TCP,
                                   kCFSocketDataCallBack,
                                   (CFSocketCallBack)&receiveData,
                                   NULL);
    ...
    sin.sin_addr = _myServerAddr;

    
    CFDataRef address, data;
    NSString *msg = [NSString stringWithFormat: @"%d", _id];
    UInt8 message[[msg length] + 1];
    memcpy(message, [msg UTF8String], [msg length] + 1);
//    NSString *message = [NSString stringWithFormat: @"%d", _id];
//    CFRunLoopSourceRef source;
    ...
    sockaddr_in6 addr;
    socklen_t l = sizeof(addr);
    int nativeSock = CFSocketGetNative(s);
    int amt = recvfrom(nativeSock,buffer,sizeof(buffer),0,(sockaddr*)&addr,&l);
    //    NSLog(@"Msg: %s", buffer);
    if (amt > 0) {
        _myMission = [NSString stringWithFormat: @"%s", buffer];
    } else {
        _myMission = [NSString stringWithFormat: @"%s", "[Err] There is problem retrieving mission."];
    }

    // Show the mission
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Secret Mission" message:_myMission delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
    [alert show];
}
//
...
- (void) leave
{
    ...
    if (_id != -1){
        ...
        //sendto(_socket,data,sizeof(data),0,(sockaddr*)&_targetAddr,_targetAddrSize);
    }
...
- (void) setDPadX:(float)x andY:(float)y andPressed:(BOOL)pressed andDirect:(BOOL)direct
{
    ...
    if (_tiltMode) fullSpeedDist = DPAD_FULL_SPEED_DIST_TILT;
    else if (_floating) fullSpeedDist = DPAD_FULL_SPEED_DIST_FLOATING;
    else fullSpeedDist = DPAD_FULL_SPEED_DIST;
...
#   define RAD_TO_DEG(A) (A*(360.0/(2.0*3.141592)))
    ...
    // lastly do some remapping before we ship this off as a state..
//    if (_tiltMode){
//        float len = sqrt(_dPadStateH*_dPadStateH+_dPadStateV*_dPadStateV);
//        if (len > 0.0){
//            float adjLen = pow(len,1.0); // remap lower values to be slower..
//            _dPadStateH *= adjLen/len;
//            _dPadStateV *= adjLen/len;
//        }
//    }
//    else{
//        if (_usingProtocolV2){
...
- (void)updateButtonsForTouches
{
    ...
    for (UITouch *touch in self.validTouches){
        ...
        if (p.x > -1.0*BUTTON_BUFFER
            ...
            if (punchLen < throwLen and punchLen < jumpLen and punchLen < bombLen) punchHeld = YES;
            else if (throwLen < punchLen and throwLen < jumpLen and throwLen < bombLen) throwHeld = YES;
            else if (jumpLen < punchLen and jumpLen < throwLen and jumpLen < bombLen) jumpHeld = YES;
            else bombHeld = YES;
        }
...
- (void)hardwareDPadChangedX:(float) x andY:(float)y{
    ...
    if (x != 0.0){
        float maxval = (x > 0.0)?1.0:-1.0;
        x = maxval*_controllerDPadSensitivity + (1.0-_controllerDPadSensitivity)*x;
    }
    if (y != 0.0){
        float maxval = (y > 0.0)?1.0:-1.0;
        y = maxval*_controllerDPadSensitivity + (1.0-_controllerDPadSensitivity)*y;
    }
    //printf("filtered: %f %f\n",x,y);
